...
/// This test makes sure that the number of DOFs in all spaces are assigned correctly.
/// Here the test using the multimesh adaptive hp-FEM example (examples/bracket,
...
int main(int argc, char* argv[])
{
  ...
  do
  {
    ...
    if (xdisp.get_max_dof() > ydisp.get_max_dof())
    {
      printf("Failure!\n");
   ...
   dof1 = ndof = assign_dofs(1, &xdisp);
   dof2 = ndof = assign_dofs(2, &xdisp, &ydisp);
    ...
    // check the sum of the dofs
    if (xdisp.get_num_dofs() + ydisp.get_num_dofs() == ndof){
      if (ydisp.get_max_dof() + 1 == ndof)
        printf("sum of dofs=%d\n", ndof);
      else{
        printf("Failure!\n");
        return ERROR_FAILURE;
      }
    ...
    if (err_est < ERR_STOP) done = true;
      ...
      done = hp.adapt(THRESHOLD, STRATEGY, ADAPT_TYPE, ISO_ONLY, MESH_REGULARITY, CONV_EXP, MAX_ORDER, SAME_ORDERS);
