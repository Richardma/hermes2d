...
// $Id: solution.cpp 1086 2008-10-21 09:05:44Z jakub $
...
/*      bool trans1 = false, trans2 = false, trans3 = false;
      ...
      {      
        for (i = 0, m = mat; i < np; i++, m += mstep)
        {
          ...
          tab1[i] = (*m)[0][0]*vx + (*m)[0][1]*vy;
          tab2[i] = (*m)[1][0]*vx + (*m)[1][1]*vy;
        }
      }
      ...
      {      
        for (i = 0, m = mat; i < np; i++, m += mstep)
        {
          scalar e0x = tab0x[i], e0y = tab0y[i];
          scalar e1x = tab1x[i], e1y = tab1y[i];
          tab1x[i] = (*m)[0][0]*((*m)[1][0]*e0x + (*m)[1][1]*e1x) + (*m)[0][1]*((*m)[1][0]*e0y + (*m)[1][1]*e1y);
